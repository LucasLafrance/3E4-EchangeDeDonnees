...
$(document).ready(() => {
    ...
    $("#btnSend").click(() => {
        const message = {
            text:$('#txtMessage').val() // objet qu'on veut envoyer
 ...
 //Réceptions des évenement
    socket.on(IOEVENTS.NEW_MESSAGE, message =>{ // message = le message que le serveur à envoyé
...
function createUserUI(user){ //elle retourne la lister des users
// user c.est le tableau qu'on a créer plsu haut
    const userLi = 
